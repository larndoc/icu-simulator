...
class hk_data:
		...
		def update(self, port, values):
			self.p = pcu_data(port.read(size = 32),  values)
			self.fi = fib_hk_data(port.read(size = 40), values)
			self.fo = fob_hk_data(port.read(size = 4),  values)
			self.fs = fsc_hk_data(port.read(size = 51), values)
		...
		while self.start_running:
			while self.serial.inWaiting():
				...
				if(decision_hk_sci == b'\x01'):
					...
					self.s = fee_science(self.time, self.serial, self.sci_values)
					for key in self.sci_values:
					...
					self.h = hk_data(self.serial, self.hk_values)
					for key in self.hk_values:
...
class fee_science():
		def __init__(self, time,  port, values):
			...
			if(header_data[0] > 0):
				self.fb = fib_sci(port.read(size = 10), time + ',' + values)
			if(header_data[1] > 0):
				self.fo = fob_sci(port.read(size = 10), time + ',' + values)
			if(header_data[2] > 0):
				self.fs = fsc_sci(port.read(size = 11), time + ',' + values)
